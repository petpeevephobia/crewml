...
`main.py` dynamically loads `01_agent_clean/agent_1.py` and prints row counts and output paths when Agent 1 finishes.
...
| `AGENT_1_DATA_CLEANING.md` | This document |
...
| Piece | Role |
|-------|------|
| `build_profile()` | Loads the full CSV; returns `(dataframe, profile_dict)` for the LLM |
| `_parse_llm_json()` | Strips optional markdown fences; parses JSON from the model |
| `_parse_european_decimal()` | Helper for `parse_european_decimal` transformations |
| `apply_transformations()` | Runs the ordered `transformations` list on a dataframe copy |
| `run()` | End-to-end: profile → LLM → apply → write CSV and report → return metadata dict |

Top-level constants (`INPUT_CSV`, `OUTPUT_CSV`, `REPORT_PATH`, `SAMPLE_ROWS`, `SYSTEM_PROMPT`) control paths and prompt text. Paths are resolved from the project root via `_ROOT`.

### `agent_1_prompt.json`

Static example of valid LLM output. Appended to the system prompt so the model mirrors structure (`issues`, `transformations`, `report_markdown`). Not executed directly — documentation and few-shot shape only.

### `cleaned.csv`

Runtime artifact. Same row count as input; columns cleaned per the LLM plan. Consumed by Agent 2 (`02_agent_feature`).

### `cleaning_report.md`

Runtime artifact. Combines the LLM’s `report_markdown`, identified `issues`, run stats, and a bullet log from `apply_transformations()`.
